...
file_topic_question = open("list_topic_question_1.txt", mode='w')
file_topic_question_log = open("list_topic_question_log_1.txt", mode='w')
topic_question = {}
	...
	for i in range(1,len(split_html)):
		...
		if ("<div") in part:
			for link in part_soup.find_all('a' , href=True):
				link_url = "http://www.quora.com" + link['href']
...
for topic in topic_question.keys():
	for question in topic_question[topic].keys():
		try:
			file_topic_question.write((question + '\n').encode('utf-8'))
			file_topic_question_log.write((question+"/log"+'\n').encode('utf-8'))
